-- ============================================
--  fcrp_dealership | server/job.lua  v2.0
--
...
RegisterNetEvent('fcrp_dealership:server:hire', function(targetId)
    local src    = source
    ...
    print("^2[fcrp_dealership] " .. bossCid .. " hired " .. targetCid .. " as Trainee^0")
end)
...
RegisterNetEvent('fcrp_dealership:server:fire', function(targetId)
    local src    = source
    if not isBoss(src) then
    ...
    -- BUG FIX: Clear the duty record so a fired employee is no longer counted
    -- as on-duty in findEmployeeAtStand. Without this, a fired player who was
    -- on duty would still pass the proximity check until they disconnect.
    FDDutyPlayers[tonumber(targetId)] = nil
    ...
    print("^2[fcrp_dealership] " .. bossCid .. " fired " .. targetCid .. "^0")
end)
...
RegisterNetEvent('fcrp_dealership:server:promote', function(targetId)
    local src    = source
    if not isBoss(src) then
...
RegisterNetEvent('fcrp_dealership:server:demote', function(targetId)
    local src    = source
    if not isBoss(src) then
...
RegisterNetEvent('fcrp_dealership:server:setDuty', function(state)
    local src    = source
    local player = QBX:GetPlayer(src)
...
lib.addCommand('fdstaff', {
    ...
    -- BUG FIX: Was QBX:GetPlayers() which is incorrect — GetPlayers is a native FiveM function
    -- and qbx_core doesn't wrap it. Use the native directly.
    ...
    TriggerClientEvent('fcrp_dealership:client:showStaffList', src, staffText, #list)
end)
...
lib.addCommand('fdsales', {
    ...
    MySQL.query([[
        ...
        FROM fcrp_dealership_sales_log
        GROUP BY citizenid
        ORDER BY sales DESC
        LIMIT 10
    ]], {}, function(result)
            ...
            MySQL.query(
                ...
                function(r)
                    ...
                    if pending == 0 then
                        ...
                        TriggerClientEvent('fcrp_dealership:client:showStaffList', src,
                            text, #result)
                    end
                end
            )
...
print("^2[fcrp_dealership] server/job.lua loaded.^0")
