-- ============================================
--  fcrp_dealership | client/job.lua  v2.0
--
...
-- ============================================
--  [FIX #14] Resource restart duty re-sync
--  client/main.lua fires setDuty(false) to the
--  server on onClientResourceStart. This handler
--  mirrors that by resetting the local isOnDuty
--  flag, so ox_target canInteract guards stay
--  correct without requiring a relog.
-- ============================================
...
local function clockIn()
    ...
    isOnDuty = true
    TriggerServerEvent('fcrp_dealership:server:setDuty', true)
...
-- BUG FIX: Was hardcoded to vec3(-950.5, -491.5, 35.84) — completely wrong location,
-- nowhere near the dealership MLO. Changed to use Config.OnDutyCoords as a sane default.
-- !! CHANGE ME !! — set to your actual boss desk position inside the MLO.
local BOSS_DESK_COORDS = Config.OnDutyCoords
            ...
            },
            {
                ...
                onSelect    = function()
                    ...
                    if input and input[1] then
                        TriggerServerEvent('fcrp_dealership:server:hire', tonumber(input[1]))
                    end
                end
            },
            {
                ...
                onSelect    = function()
                    ...
                    if input and input[1] then
                        TriggerServerEvent('fcrp_dealership:server:fire', tonumber(input[1]))
                    end
                end
            },
            {
                ...
                onSelect    = function()
                    ...
                    if input and input[1] then
                        TriggerServerEvent('fcrp_dealership:server:promote', tonumber(input[1]))
                    end
                end
            },
            {
                ...
                onSelect    = function()
                    ...
                    if input and input[1] then
                        TriggerServerEvent('fcrp_dealership:server:demote', tonumber(input[1]))
                    end
                end
            },
            {
...
RegisterNetEvent('fcrp_dealership:client:receiveSocietyBalance', function(balance)
        ...
        options = {
            ...
            },
            {
                ...
                onSelect    = function()
                    ...
                    if input and input[1] then
                        TriggerServerEvent('fcrp_dealership:server:withdrawSociety', tonumber(input[1]))
                    end
                end
            },
        }
...
exports.ox_target:addGlobalPlayer({
    {
        ...
        onSelect = function(data)
            -- BUG FIX: Added IsPedAPlayer check — without it a salesperson could
            -- accidentally target an NPC ped and send a test drive event for them,
            -- which would error server-side when GetPlayer(npcServerId) returns nil.
            if not IsPedAPlayer(data.entity) then
                lib.notify({ type = 'error', description = 'Target is not a player.' })
                return
            end
            ...
            if input and input[1] then
                ...
                if Config.Vehicles[idx] then
                    TriggerServerEvent('fcrp_dealership:server:startTestDrive', Config.Vehicles[idx].model, targetServerId)
                end
            end
        end
    }
...
print("^2[fcrp_dealership] client/job.lua loaded.^0")
